...
func TestRepository_Create(t *testing.T) {
	repo := sale_item.NewRepository(testDB, testLogger)
	ctx := context.Background()

	tests := []struct {
		name    string
		entity  *sale_item.SaleItems
		wantErr bool
	}{
		{
			name:    "valid entity",
			entity:  newTestSaleItems(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestSaleItemsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestSaleItems(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := sale_item.NewRepository(testDB, testLogger)
	service := sale_item.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateSaleItemsRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestSaleItemsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestSaleItemsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := sale_item.NewRepository(testDB, testLogger)
	service := sale_item.NewService(testDB, repo, testLogger)
	handler := sale_item.NewHandler(service, testLogger)

	reqBody := &dto.CreateSaleItemsRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/sale_items", strings.NewReader(string(body)))
	...
	var resp dto.SaleItemsResponse
...
// newTestSaleItems creates a test SaleItems entity
func newTestSaleItems(orgID uuid.UUID) *sale_item.SaleItems {
	return &sale_item.SaleItems{
...
// newTestSaleItemsWithID creates a test SaleItems entity with specific ID
func newTestSaleItemsWithID(id uuid.UUID, orgID uuid.UUID) *sale_item.SaleItems {
	entity := newTestSaleItems(orgID)
	entity.ID = id
	return entity
}

// newTestSaleItemsRequest creates a test create request
func newTestSaleItemsRequest(orgID uuid.UUID) *dto.CreateSaleItemsRequest {
	return &dto.CreateSaleItemsRequest{

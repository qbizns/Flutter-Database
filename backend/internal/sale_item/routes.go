...
// RegisterRoutes registers all SaleItems routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/sale_items", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public SaleItems routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/sale_items", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only SaleItems routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/sale_items", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

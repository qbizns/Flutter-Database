...
// RegisterRoutes registers all AnalyticAccounts routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/analytic_accounts", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public AnalyticAccounts routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/analytic_accounts", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only AnalyticAccounts routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/analytic_accounts", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all Organizations routes
...
// RegisterPublicRoutes registers public Organizations routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/organizations", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Organizations routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/organizations", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

...
// RegisterRoutes registers all SmsQueue routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/sms_queue", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public SmsQueue routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/sms_queue", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only SmsQueue routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/sms_queue", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

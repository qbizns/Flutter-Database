...
// RegisterRoutes registers all StoreCreditTransactions routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/store_credit_transactions", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public StoreCreditTransactions routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/store_credit_transactions", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only StoreCreditTransactions routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/store_credit_transactions", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

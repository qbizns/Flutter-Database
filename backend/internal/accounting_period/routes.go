...
// RegisterRoutes registers all AccountingPeriods routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/accounting_periods", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public AccountingPeriods routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/accounting_periods", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only AccountingPeriods routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/accounting_periods", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

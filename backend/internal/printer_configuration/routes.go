...
// RegisterRoutes registers all PrinterConfigurations routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/printer_configurations", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public PrinterConfigurations routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/printer_configurations", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only PrinterConfigurations routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/printer_configurations", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all InventoryValuationSettings routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/inventory_valuation_settings", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public InventoryValuationSettings routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/inventory_valuation_settings", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only InventoryValuationSettings routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/inventory_valuation_settings", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

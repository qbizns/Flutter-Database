...
// NewValidator creates a new CycleCountItems validator
...
func (v *Validator) ValidateCreate(ctx context.Context, tx pgx.Tx, req *dto.CreateCycleCountItemsRequest) error {
	...
	// Validate CycleCountId
	
	
	if err := v.validateCycleCountIdExists(ctx, tx, req.CycleCountId); err != nil {
		return err
	}
	...
	// Validate RecountRequired
	
	
	
	// Validate RecountQuantity
	
	
	
	// Validate RecountReason
	
	
	
	// Validate AdjustmentApplied
...
func (v *Validator) ValidateUpdate(ctx context.Context, tx pgx.Tx, id uuid.UUID, req *dto.UpdateCycleCountItemsRequest) error {
	...
	// Validate CycleCountId if provided
	...
	// Validate SystemQuantity if provided
	
	
	
	// Validate CountedQuantity if provided
	...
	// Validate VariancePercentage if provided
	...
	// Validate VarianceValue if provided
	...
	// Validate RecountRequired if provided
	
	
	
	// Validate RecountQuantity if provided
	
	
	
	// Validate RecountReason if provided
	
	
	
	// Validate AdjustmentApplied if provided
	
	
	
	// Validate AdjustmentDate if provided
	
	
	
	// Validate AdjustmentReason if provided
	...
	// Validate CountedAt if provided
...
// validateCycleCountIdExists validates that cycle_count_id exists
func (v *Validator) validateCycleCountIdExists(ctx context.Context, tx pgx.Tx, id uuid.UUID) error {
	// TODO: Implement existence check for cycle_count
	// This should query the referenced table to ensure the ID exists
	var exists bool
	err := tx.QueryRow(ctx, "SELECT EXISTS(SELECT 1 FROM cycle_count WHERE id = $1)", id).Scan(&exists)
	if err != nil {
		return fmt.Errorf("failed to check cycle_count existence: %w", err)
	}
	if !exists {
		return fmt.Errorf("cycle_count with id %s does not exist", id)
	}
	return nil
}
...
func (v *Validator) validateCanDelete(ctx context.Context, tx pgx.Tx, entity *CycleCountItems) error {

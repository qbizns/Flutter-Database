...
// RegisterRoutes registers all BankReconciliationItems routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/bank_reconciliation_items", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public BankReconciliationItems routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/bank_reconciliation_items", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only BankReconciliationItems routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/bank_reconciliation_items", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

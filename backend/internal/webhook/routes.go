...
// RegisterRoutes registers all Webhooks routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/webhooks", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public Webhooks routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/webhooks", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Webhooks routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/webhooks", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

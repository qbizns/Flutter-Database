package bank_statement_reconciliation_test
...
func TestRepository_Create(t *testing.T) {
	repo := bank_statement_reconciliation.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestBankStatementReconciliations(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestBankStatementReconciliationsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestBankStatementReconciliations(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestBankStatementReconciliationsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestBankStatementReconciliationsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/bank_statement_reconciliations", strings.NewReader(string(body)))
...
// newTestBankStatementReconciliations creates a test BankStatementReconciliations entity
func newTestBankStatementReconciliations(orgID uuid.UUID) *bank_statement_reconciliation.BankStatementReconciliations {
	return &bank_statement_reconciliation.BankStatementReconciliations{
...
// newTestBankStatementReconciliationsWithID creates a test BankStatementReconciliations entity with specific ID
func newTestBankStatementReconciliationsWithID(id uuid.UUID, orgID uuid.UUID) *bank_statement_reconciliation.BankStatementReconciliations {
	entity := newTestBankStatementReconciliations(orgID)
	entity.ID = id
	return entity
}

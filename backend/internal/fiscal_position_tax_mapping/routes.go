...
// RegisterRoutes registers all FiscalPositionTaxMappings routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/fiscal_position_tax_mappings", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public FiscalPositionTaxMappings routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/fiscal_position_tax_mappings", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only FiscalPositionTaxMappings routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/fiscal_position_tax_mappings", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

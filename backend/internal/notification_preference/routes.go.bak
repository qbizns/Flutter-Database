...
// RegisterRoutes registers all NotificationPreferences routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/notification_preferences", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public NotificationPreferences routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/notification_preferences", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only NotificationPreferences routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/notification_preferences", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all DeliveryZones routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/delivery_zones", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public DeliveryZones routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/delivery_zones", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only DeliveryZones routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/delivery_zones", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

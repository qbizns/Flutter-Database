...
// RegisterRoutes registers all DeliveryAssignments routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/delivery_assignments", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public DeliveryAssignments routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/delivery_assignments", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only DeliveryAssignments routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/delivery_assignments", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

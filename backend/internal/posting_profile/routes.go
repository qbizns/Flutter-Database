...
// RegisterRoutes registers all PostingProfiles routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/posting_profiles", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public PostingProfiles routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/posting_profiles", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only PostingProfiles routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/posting_profiles", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

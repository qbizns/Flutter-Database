...
func TestRepository_Create(t *testing.T) {
	repo := user_session.NewRepository(testDB, testLogger)
	ctx := context.Background()

	tests := []struct {
		name    string
		entity  *user_session.UserSessions
		wantErr bool
	}{
		{
			name:    "valid entity",
			entity:  newTestUserSessions(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestUserSessionsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestUserSessions(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := user_session.NewRepository(testDB, testLogger)
	service := user_session.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateUserSessionsRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestUserSessionsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestUserSessionsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := user_session.NewRepository(testDB, testLogger)
	service := user_session.NewService(testDB, repo, testLogger)
	handler := user_session.NewHandler(service, testLogger)

	reqBody := &dto.CreateUserSessionsRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/user_sessions", strings.NewReader(string(body)))
...
// newTestUserSessions creates a test UserSessions entity
func newTestUserSessions(orgID uuid.UUID) *user_session.UserSessions {
	return &user_session.UserSessions{
...
// newTestUserSessionsWithID creates a test UserSessions entity with specific ID
func newTestUserSessionsWithID(id uuid.UUID, orgID uuid.UUID) *user_session.UserSessions {
	entity := newTestUserSessions(orgID)
	entity.ID = id
	return entity
}

// newTestUserSessionsRequest creates a test create request
func newTestUserSessionsRequest(orgID uuid.UUID) *dto.CreateUserSessionsRequest {
	return &dto.CreateUserSessionsRequest{

...
// RegisterRoutes registers all PosPostingAudit routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/pos_posting_audit", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public PosPostingAudit routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/pos_posting_audit", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only PosPostingAudit routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/pos_posting_audit", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

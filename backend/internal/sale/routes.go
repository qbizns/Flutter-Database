...
func RegisterRoutes(r chi.Router, handler *Handler) {
	...
	r.Route("/api/v1/organizations/{orgID}/sales", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public Sales routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/sales", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Sales routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/sales", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

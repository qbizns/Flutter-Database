...
func TestRepository_Create(t *testing.T) {
	repo := sale.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestSales(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestSalesWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_Update(t *testing.T) {
	...
	entity.SaleNumber = "updated_value"
	...
	assert.Equal(t, "updated_value", updated.SaleNumber)
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestSales(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := sale.NewRepository(testDB, testLogger)
	service := sale.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateSalesRequest{
		
		SaleNumber: "test_value",
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestSalesRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestSalesRequest(testOrgID)
	...
	updateReq := &dto.UpdateSalesRequest{
		SaleNumber: &updatedValue,
	}
...
func TestHandler_Create(t *testing.T) {
	repo := sale.NewRepository(testDB, testLogger)
	service := sale.NewService(testDB, repo, testLogger)
	handler := sale.NewHandler(service, testLogger)

	reqBody := &dto.CreateSalesRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/sales", strings.NewReader(string(body)))
...
// newTestSales creates a test Sales entity
func newTestSales(orgID uuid.UUID) *sale.Sales {
	return &sale.Sales{
...
// newTestSalesWithID creates a test Sales entity with specific ID
func newTestSalesWithID(id uuid.UUID, orgID uuid.UUID) *sale.Sales {
	entity := newTestSales(orgID)
	entity.ID = id
	return entity
}

// newTestSalesRequest creates a test create request
func newTestSalesRequest(orgID uuid.UUID) *dto.CreateSalesRequest {
	return &dto.CreateSalesRequest{

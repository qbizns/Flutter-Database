...
// RegisterRoutes registers all PostingRules routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/posting_rules", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public PostingRules routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/posting_rules", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only PostingRules routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/posting_rules", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

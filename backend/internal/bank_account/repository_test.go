...
func TestRepository_Create(t *testing.T) {
	repo := bank_account.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestBankAccounts(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestBankAccountsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestBankAccounts(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := bank_account.NewRepository(testDB, testLogger)
	service := bank_account.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateBankAccountsRequest{
		
		
		
		
		ChartAccountId: uuid.New(),
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestBankAccountsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestBankAccountsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := bank_account.NewRepository(testDB, testLogger)
	service := bank_account.NewService(testDB, repo, testLogger)
	handler := bank_account.NewHandler(service, testLogger)

	reqBody := &dto.CreateBankAccountsRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/bank_accounts", strings.NewReader(string(body)))
	...
	var resp dto.BankAccountsResponse
...
// newTestBankAccounts creates a test BankAccounts entity
func newTestBankAccounts(orgID uuid.UUID) *bank_account.BankAccounts {
	return &bank_account.BankAccounts{
...
// newTestBankAccountsWithID creates a test BankAccounts entity with specific ID
func newTestBankAccountsWithID(id uuid.UUID, orgID uuid.UUID) *bank_account.BankAccounts {
	entity := newTestBankAccounts(orgID)
	entity.ID = id
	return entity
}

// newTestBankAccountsRequest creates a test create request
func newTestBankAccountsRequest(orgID uuid.UUID) *dto.CreateBankAccountsRequest {
	return &dto.CreateBankAccountsRequest{

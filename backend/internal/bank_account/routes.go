...
// RegisterRoutes registers all BankAccounts routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/bank_accounts", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public BankAccounts routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/bank_accounts", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only BankAccounts routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/bank_accounts", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

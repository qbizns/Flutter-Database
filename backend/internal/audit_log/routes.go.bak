...
// RegisterRoutes registers all AuditLogs routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/audit_logs", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public AuditLogs routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/audit_logs", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only AuditLogs routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/audit_logs", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

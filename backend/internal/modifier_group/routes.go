...
// RegisterRoutes registers all ModifierGroups routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/modifier_groups", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public ModifierGroups routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/modifier_groups", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only ModifierGroups routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/modifier_groups", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

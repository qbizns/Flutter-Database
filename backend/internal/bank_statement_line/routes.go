...
// RegisterRoutes registers all BankStatementLines routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/bank_statement_lines", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public BankStatementLines routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/bank_statement_lines", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only BankStatementLines routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/bank_statement_lines", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

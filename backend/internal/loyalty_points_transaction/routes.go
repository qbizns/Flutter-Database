...
// RegisterRoutes registers all LoyaltyPointsTransactions routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/loyalty_points_transactions", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public LoyaltyPointsTransactions routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/loyalty_points_transactions", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only LoyaltyPointsTransactions routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/loyalty_points_transactions", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

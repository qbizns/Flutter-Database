...
// RegisterRoutes registers all OrganizationFeatures routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/organization_features", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public OrganizationFeatures routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/organization_features", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only OrganizationFeatures routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/organization_features", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

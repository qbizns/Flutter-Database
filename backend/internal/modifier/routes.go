...
// RegisterRoutes registers all Modifiers routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/modifiers", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public Modifiers routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/modifiers", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Modifiers routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/modifiers", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

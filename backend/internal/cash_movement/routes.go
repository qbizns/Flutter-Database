...
// RegisterRoutes registers all CashMovements routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/cash_movements", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public CashMovements routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/cash_movements", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only CashMovements routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/cash_movements", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

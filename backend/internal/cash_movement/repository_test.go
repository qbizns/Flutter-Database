...
func TestRepository_Create(t *testing.T) {
	repo := cash_movement.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestCashMovements(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestCashMovementsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestCashMovements(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestCashMovementsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestCashMovementsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/cash_movements", strings.NewReader(string(body)))
...
// newTestCashMovements creates a test CashMovements entity
func newTestCashMovements(orgID uuid.UUID) *cash_movement.CashMovements {
	return &cash_movement.CashMovements{
		...
		ReasonDescription: "test_reason_description",
...
// newTestCashMovementsWithID creates a test CashMovements entity with specific ID
func newTestCashMovementsWithID(id uuid.UUID, orgID uuid.UUID) *cash_movement.CashMovements {
	entity := newTestCashMovements(orgID)
	entity.ID = id
	return entity
}

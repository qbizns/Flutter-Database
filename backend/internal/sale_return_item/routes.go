...
// RegisterRoutes registers all SaleReturnItems routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/sale_return_items", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public SaleReturnItems routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/sale_return_items", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only SaleReturnItems routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/sale_return_items", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

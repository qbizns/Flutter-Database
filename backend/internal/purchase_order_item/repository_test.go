package purchase_order_item_test
...
func TestRepository_Create(t *testing.T) {
	repo := purchase_order_item.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestPurchaseOrderItems(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestPurchaseOrderItemsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestPurchaseOrderItems(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestPurchaseOrderItemsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestPurchaseOrderItemsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/purchase_order_items", strings.NewReader(string(body)))
...
// newTestPurchaseOrderItems creates a test PurchaseOrderItems entity
func newTestPurchaseOrderItems(orgID uuid.UUID) *purchase_order_item.PurchaseOrderItems {
	return &purchase_order_item.PurchaseOrderItems{
...
// newTestPurchaseOrderItemsWithID creates a test PurchaseOrderItems entity with specific ID
func newTestPurchaseOrderItemsWithID(id uuid.UUID, orgID uuid.UUID) *purchase_order_item.PurchaseOrderItems {
	entity := newTestPurchaseOrderItems(orgID)
	entity.ID = id
	return entity
}

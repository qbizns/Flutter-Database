...
// RegisterRoutes registers all TaxReportLines routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/tax_report_lines", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public TaxReportLines routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/tax_report_lines", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only TaxReportLines routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/tax_report_lines", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

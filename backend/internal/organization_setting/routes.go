...
// RegisterRoutes registers all OrganizationSettings routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/organization_settings", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public OrganizationSettings routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/organization_settings", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only OrganizationSettings routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/organization_settings", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

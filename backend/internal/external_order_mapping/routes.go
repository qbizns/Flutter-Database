...
// RegisterRoutes registers all ExternalOrderMappings routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/external_order_mappings", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public ExternalOrderMappings routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/external_order_mappings", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only ExternalOrderMappings routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/external_order_mappings", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

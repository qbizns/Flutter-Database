...
// RegisterRoutes registers all PriceListItems routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/price_list_items", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public PriceListItems routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/price_list_items", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only PriceListItems routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/price_list_items", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

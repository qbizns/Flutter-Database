...
// RegisterRoutes registers all BankStatements routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/bank_statements", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public BankStatements routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/bank_statements", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only BankStatements routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/bank_statements", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

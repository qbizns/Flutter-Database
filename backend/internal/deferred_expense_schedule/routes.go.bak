...
// RegisterRoutes registers all DeferredExpenseSchedule routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/deferred_expense_schedule", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public DeferredExpenseSchedule routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/deferred_expense_schedule", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only DeferredExpenseSchedule routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/deferred_expense_schedule", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all JournalEntryTypes routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/journal_entry_types", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public JournalEntryTypes routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/journal_entry_types", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only JournalEntryTypes routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/journal_entry_types", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all RestaurantTables routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/restaurant_tables", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public RestaurantTables routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/restaurant_tables", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only RestaurantTables routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/restaurant_tables", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

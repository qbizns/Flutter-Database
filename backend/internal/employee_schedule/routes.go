...
// RegisterRoutes registers all EmployeeSchedules routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/employee_schedules", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public EmployeeSchedules routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/employee_schedules", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only EmployeeSchedules routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/employee_schedules", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

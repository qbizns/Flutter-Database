...
// Validator handles GiftCardTransactions validation logic
type Validator struct {
	repo *Repository
}

// NewValidator creates a new GiftCardTransactions validator
...
func (v *Validator) ValidateCreate(ctx context.Context, tx pgx.Tx, req *dto.CreateGiftCardTransactionsRequest) error {
	// Basic validation
	...
	if err := v.validateGiftCardIdExists(ctx, tx, req.GiftCardId); err != nil {
		return err
	}
...
func (v *Validator) ValidateUpdate(ctx context.Context, tx pgx.Tx, id uuid.UUID, req *dto.UpdateGiftCardTransactionsRequest) error {
	// Basic validation
	if err := req.Validate(); err != nil {
		return fmt.Errorf("validation failed: %w", err)
	}
	...
	// Validate GiftCardId if provided
...
// validateGiftCardIdExists validates that gift_card_id exists
func (v *Validator) validateGiftCardIdExists(ctx context.Context, tx pgx.Tx, id uuid.UUID) error {
	// TODO: Implement existence check for gift_card
	// This should query the referenced table to ensure the ID exists
	var exists bool
	err := tx.QueryRow(ctx, "SELECT EXISTS(SELECT 1 FROM gift_card WHERE id = $1)", id).Scan(&exists)
	if err != nil {
		return fmt.Errorf("failed to check gift_card existence: %w", err)
	}
	if !exists {
		return fmt.Errorf("gift_card with id %s does not exist", id)
	}
	return nil
}
...
func (v *Validator) validateCrossFields(ctx context.Context, tx pgx.Tx, req *dto.CreateGiftCardTransactionsRequest) error {
...
func (v *Validator) validateBusinessRules(ctx context.Context, tx pgx.Tx, req *dto.CreateGiftCardTransactionsRequest) error {
...
func (v *Validator) validateUpdateBusinessRules(ctx context.Context, tx pgx.Tx, existing *GiftCardTransactions, req *dto.UpdateGiftCardTransactionsRequest) error {
...
func (v *Validator) validateCanDelete(ctx context.Context, tx pgx.Tx, entity *GiftCardTransactions) error {

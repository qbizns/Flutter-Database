...
// RegisterRoutes registers all GiftCardTransactions routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/gift_card_transactions", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public GiftCardTransactions routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/gift_card_transactions", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only GiftCardTransactions routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/gift_card_transactions", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

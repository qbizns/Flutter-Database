...
// RegisterRoutes registers all CustomerPaymentApplications routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/customer_payment_applications", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public CustomerPaymentApplications routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/customer_payment_applications", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only CustomerPaymentApplications routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/customer_payment_applications", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

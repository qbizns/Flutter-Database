...
// RegisterRoutes registers all AccountSubtypes routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/account_subtypes", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public AccountSubtypes routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/account_subtypes", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only AccountSubtypes routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/account_subtypes", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all PostingValidationResults routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/posting_validation_results", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public PostingValidationResults routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/posting_validation_results", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only PostingValidationResults routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/posting_validation_results", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

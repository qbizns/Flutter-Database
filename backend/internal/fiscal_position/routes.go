...
// RegisterRoutes registers all FiscalPositions routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/fiscal_positions", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public FiscalPositions routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/fiscal_positions", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only FiscalPositions routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/fiscal_positions", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

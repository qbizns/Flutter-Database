...
// RegisterRoutes registers all LocalizationPackages routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/localization_packages", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public LocalizationPackages routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/localization_packages", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only LocalizationPackages routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/localization_packages", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

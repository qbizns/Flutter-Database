package inventory_transfer_test
...
func TestRepository_Create(t *testing.T) {
	repo := inventory_transfer.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestInventoryTransfers(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestInventoryTransfersWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_Update(t *testing.T) {
	...
	entity.TransferNumber = "updated_value"
	...
	assert.Equal(t, "updated_value", updated.TransferNumber)
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestInventoryTransfers(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestInventoryTransfersRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestInventoryTransfersRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/inventory_transfers", strings.NewReader(string(body)))
...
// newTestInventoryTransfers creates a test InventoryTransfers entity
func newTestInventoryTransfers(orgID uuid.UUID) *inventory_transfer.InventoryTransfers {
	return &inventory_transfer.InventoryTransfers{
...
// newTestInventoryTransfersWithID creates a test InventoryTransfers entity with specific ID
func newTestInventoryTransfersWithID(id uuid.UUID, orgID uuid.UUID) *inventory_transfer.InventoryTransfers {
	entity := newTestInventoryTransfers(orgID)
	entity.ID = id
	return entity
}

...
// RegisterRoutes registers all ChartOfAccounts routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/chart_of_accounts", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public ChartOfAccounts routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/chart_of_accounts", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only ChartOfAccounts routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/chart_of_accounts", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

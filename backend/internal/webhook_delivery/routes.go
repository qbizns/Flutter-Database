...
// RegisterRoutes registers all WebhookDeliveries routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/webhook_deliveries", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public WebhookDeliveries routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/webhook_deliveries", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only WebhookDeliveries routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/webhook_deliveries", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

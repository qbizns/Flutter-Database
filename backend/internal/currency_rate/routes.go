...
// RegisterRoutes registers all CurrencyRates routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/currency_rates", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public CurrencyRates routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/currency_rates", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only CurrencyRates routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/currency_rates", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

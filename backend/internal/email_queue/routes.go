...
// RegisterRoutes registers all EmailQueue routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/email_queue", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public EmailQueue routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/email_queue", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only EmailQueue routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/email_queue", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all PaymentTerms routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/payment_terms", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public PaymentTerms routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/payment_terms", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only PaymentTerms routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/payment_terms", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

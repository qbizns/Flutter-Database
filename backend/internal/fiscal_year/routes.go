...
// RegisterRoutes registers all FiscalYears routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/fiscal_years", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public FiscalYears routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/fiscal_years", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only FiscalYears routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/fiscal_years", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

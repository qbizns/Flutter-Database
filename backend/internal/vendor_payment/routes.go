...
// RegisterRoutes registers all VendorPayments routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/vendor_payments", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public VendorPayments routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/vendor_payments", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only VendorPayments routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/vendor_payments", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

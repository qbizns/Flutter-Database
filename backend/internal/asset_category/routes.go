...
// RegisterRoutes registers all AssetCategories routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/asset_categories", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public AssetCategories routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/asset_categories", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only AssetCategories routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/asset_categories", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all PostingValidationRules routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/posting_validation_rules", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public PostingValidationRules routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/posting_validation_rules", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only PostingValidationRules routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/posting_validation_rules", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

...
// RegisterRoutes registers all AnalyticPlans routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/analytic_plans", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public AnalyticPlans routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/analytic_plans", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only AnalyticPlans routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/analytic_plans", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

...
// RegisterRoutes registers all ApiRequestLogs routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/api_request_logs", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public ApiRequestLogs routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/api_request_logs", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only ApiRequestLogs routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/api_request_logs", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all VendorPaymentApplications routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/vendor_payment_applications", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public VendorPaymentApplications routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/vendor_payment_applications", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only VendorPaymentApplications routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/vendor_payment_applications", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

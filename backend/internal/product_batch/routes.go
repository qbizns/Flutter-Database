...
// RegisterRoutes registers all ProductBatches routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/product_batches", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public ProductBatches routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/product_batches", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only ProductBatches routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/product_batches", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

...
// RegisterRoutes registers all GiftCards routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/gift_cards", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public GiftCards routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/gift_cards", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only GiftCards routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/gift_cards", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

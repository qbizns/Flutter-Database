...
// RegisterRoutes registers all Journals routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/journals", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public Journals routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/journals", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Journals routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/journals", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all Suppliers routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/suppliers", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public Suppliers routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/suppliers", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Suppliers routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/suppliers", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

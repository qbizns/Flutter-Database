...
// RegisterRoutes registers all CycleCounts routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/cycle_counts", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public CycleCounts routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/cycle_counts", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only CycleCounts routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/cycle_counts", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

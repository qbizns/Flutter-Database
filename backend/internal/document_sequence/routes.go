...
// RegisterRoutes registers all DocumentSequences routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/document_sequences", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public DocumentSequences routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/document_sequences", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only DocumentSequences routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/document_sequences", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all FloorPlans routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/floor_plans", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public FloorPlans routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/floor_plans", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only FloorPlans routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/floor_plans", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

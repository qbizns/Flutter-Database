...
func TestRepository_Create(t *testing.T) {
	repo := floor_plan.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestFloorPlans(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestFloorPlansWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestFloorPlans(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := floor_plan.NewRepository(testDB, testLogger)
	service := floor_plan.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateFloorPlansRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestFloorPlansRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestFloorPlansRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := floor_plan.NewRepository(testDB, testLogger)
	service := floor_plan.NewService(testDB, repo, testLogger)
	handler := floor_plan.NewHandler(service, testLogger)

	reqBody := &dto.CreateFloorPlansRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/floor_plans", strings.NewReader(string(body)))
	...
	var resp dto.FloorPlansResponse
...
// newTestFloorPlans creates a test FloorPlans entity
func newTestFloorPlans(orgID uuid.UUID) *floor_plan.FloorPlans {
	return &floor_plan.FloorPlans{
...
// newTestFloorPlansWithID creates a test FloorPlans entity with specific ID
func newTestFloorPlansWithID(id uuid.UUID, orgID uuid.UUID) *floor_plan.FloorPlans {
	entity := newTestFloorPlans(orgID)
	entity.ID = id
	return entity
}

// newTestFloorPlansRequest creates a test create request
func newTestFloorPlansRequest(orgID uuid.UUID) *dto.CreateFloorPlansRequest {
	return &dto.CreateFloorPlansRequest{

...
// RegisterRoutes registers all UserSettings routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/user_settings", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public UserSettings routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/user_settings", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only UserSettings routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/user_settings", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

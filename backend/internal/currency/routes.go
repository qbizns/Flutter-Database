...
// RegisterRoutes registers all Currencies routes
...
// RegisterPublicRoutes registers public Currencies routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/currencies", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Currencies routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/currencies", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

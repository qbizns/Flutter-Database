...
// RegisterRoutes registers all ReconciliationRuleModels routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/reconciliation_rule_models", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public ReconciliationRuleModels routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/reconciliation_rule_models", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only ReconciliationRuleModels routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/reconciliation_rule_models", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

package reconciliation_rule_model_test
...
func TestRepository_Create(t *testing.T) {
	repo := reconciliation_rule_model.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestReconciliationRuleModels(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestReconciliationRuleModelsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_Update(t *testing.T) {
	...
	assert.Equal(t, "updated_value", updated.RuleName)
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestReconciliationRuleModels(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestReconciliationRuleModelsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestReconciliationRuleModelsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/reconciliation_rule_models", strings.NewReader(string(body)))
...
// newTestReconciliationRuleModels creates a test ReconciliationRuleModels entity
func newTestReconciliationRuleModels(orgID uuid.UUID) *reconciliation_rule_model.ReconciliationRuleModels {
	return &reconciliation_rule_model.ReconciliationRuleModels{
...
// newTestReconciliationRuleModelsWithID creates a test ReconciliationRuleModels entity with specific ID
func newTestReconciliationRuleModelsWithID(id uuid.UUID, orgID uuid.UUID) *reconciliation_rule_model.ReconciliationRuleModels {
	entity := newTestReconciliationRuleModels(orgID)
	entity.ID = id
	return entity
}

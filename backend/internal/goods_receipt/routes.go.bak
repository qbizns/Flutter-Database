...
// RegisterRoutes registers all GoodsReceipts routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/goods_receipts", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public GoodsReceipts routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/goods_receipts", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only GoodsReceipts routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/goods_receipts", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

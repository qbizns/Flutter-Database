...
// RegisterRoutes registers all ProductSerialNumbers routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/product_serial_numbers", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public ProductSerialNumbers routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/product_serial_numbers", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only ProductSerialNumbers routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/product_serial_numbers", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all Devices routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/devices", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public Devices routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/devices", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Devices routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/devices", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

...
func TestRepository_Create(t *testing.T) {
	repo := device.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestDevices(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestDevicesWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestDevices(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := device.NewRepository(testDB, testLogger)
	service := device.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateDevicesRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestDevicesRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestDevicesRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := device.NewRepository(testDB, testLogger)
	service := device.NewService(testDB, repo, testLogger)
	handler := device.NewHandler(service, testLogger)

	reqBody := &dto.CreateDevicesRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/devices", strings.NewReader(string(body)))
...
// newTestDevices creates a test Devices entity
func newTestDevices(orgID uuid.UUID) *device.Devices {
...
// newTestDevicesWithID creates a test Devices entity with specific ID
func newTestDevicesWithID(id uuid.UUID, orgID uuid.UUID) *device.Devices {
	entity := newTestDevices(orgID)
	entity.ID = id
	return entity
}

// newTestDevicesRequest creates a test create request
func newTestDevicesRequest(orgID uuid.UUID) *dto.CreateDevicesRequest {
	return &dto.CreateDevicesRequest{

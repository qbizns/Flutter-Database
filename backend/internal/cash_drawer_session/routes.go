...
// RegisterRoutes registers all CashDrawerSessions routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/cash_drawer_sessions", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public CashDrawerSessions routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/cash_drawer_sessions", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only CashDrawerSessions routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/cash_drawer_sessions", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
func TestRepository_Create(t *testing.T) {
	repo := api_key.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestApiKeys(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestApiKeysWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_Update(t *testing.T) {
	...
	assert.Equal(t, "updated_value", updated.KeyName)
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestApiKeys(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := api_key.NewRepository(testDB, testLogger)
	service := api_key.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateApiKeysRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestApiKeysRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestApiKeysRequest(testOrgID)
	...
	updateReq := &dto.UpdateApiKeysRequest{
...
func TestHandler_Create(t *testing.T) {
	repo := api_key.NewRepository(testDB, testLogger)
	service := api_key.NewService(testDB, repo, testLogger)
	handler := api_key.NewHandler(service, testLogger)

	reqBody := &dto.CreateApiKeysRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/api_keys", strings.NewReader(string(body)))
...
// newTestApiKeys creates a test ApiKeys entity
func newTestApiKeys(orgID uuid.UUID) *api_key.ApiKeys {
	return &api_key.ApiKeys{
...
// newTestApiKeysWithID creates a test ApiKeys entity with specific ID
func newTestApiKeysWithID(id uuid.UUID, orgID uuid.UUID) *api_key.ApiKeys {
	entity := newTestApiKeys(orgID)
	entity.ID = id
	return entity
}

// newTestApiKeysRequest creates a test create request
func newTestApiKeysRequest(orgID uuid.UUID) *dto.CreateApiKeysRequest {
	return &dto.CreateApiKeysRequest{

...
// RegisterRoutes registers all ApiKeys routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/api_keys", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public ApiKeys routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/api_keys", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only ApiKeys routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/api_keys", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

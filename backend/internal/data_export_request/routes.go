...
// RegisterRoutes registers all DataExportRequests routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/data_export_requests", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public DataExportRequests routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/data_export_requests", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only DataExportRequests routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/data_export_requests", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

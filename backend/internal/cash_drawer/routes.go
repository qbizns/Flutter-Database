...
// RegisterRoutes registers all CashDrawers routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/cash_drawers", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public CashDrawers routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/cash_drawers", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only CashDrawers routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/cash_drawers", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

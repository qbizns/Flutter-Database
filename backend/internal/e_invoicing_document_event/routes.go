...
// RegisterRoutes registers all EInvoicingDocumentEvents routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/e_invoicing_document_events", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public EInvoicingDocumentEvents routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/e_invoicing_document_events", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only EInvoicingDocumentEvents routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/e_invoicing_document_events", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

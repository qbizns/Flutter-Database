...
// RegisterRoutes registers all ReturnReasons routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/return_reasons", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public ReturnReasons routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/return_reasons", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only ReturnReasons routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/return_reasons", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

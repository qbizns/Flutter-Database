...
func TestRepository_Create(t *testing.T) {
	repo := payment.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestPayments(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestPaymentsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestPayments(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := payment.NewRepository(testDB, testLogger)
	service := payment.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreatePaymentsRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestPaymentsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestPaymentsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := payment.NewRepository(testDB, testLogger)
	service := payment.NewService(testDB, repo, testLogger)
	handler := payment.NewHandler(service, testLogger)

	reqBody := &dto.CreatePaymentsRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/payments", strings.NewReader(string(body)))
...
// newTestPayments creates a test Payments entity
func newTestPayments(orgID uuid.UUID) *payment.Payments {
...
// newTestPaymentsWithID creates a test Payments entity with specific ID
func newTestPaymentsWithID(id uuid.UUID, orgID uuid.UUID) *payment.Payments {
	entity := newTestPayments(orgID)
	entity.ID = id
	return entity
}

// newTestPaymentsRequest creates a test create request
func newTestPaymentsRequest(orgID uuid.UUID) *dto.CreatePaymentsRequest {
	return &dto.CreatePaymentsRequest{

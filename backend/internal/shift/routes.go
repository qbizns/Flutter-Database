...
// RegisterRoutes registers all Shifts routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/shifts", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public Shifts routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/shifts", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Shifts routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/shifts", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

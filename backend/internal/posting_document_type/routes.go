...
// RegisterRoutes registers all PostingDocumentTypes routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/posting_document_types", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public PostingDocumentTypes routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/posting_document_types", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only PostingDocumentTypes routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/posting_document_types", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all YourTableName routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/your_table_name", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public YourTableName routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/your_table_name", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only YourTableName routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/your_table_name", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

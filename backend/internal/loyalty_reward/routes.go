...
// RegisterRoutes registers all LoyaltyRewards routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/loyalty_rewards", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public LoyaltyRewards routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/loyalty_rewards", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only LoyaltyRewards routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/loyalty_rewards", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

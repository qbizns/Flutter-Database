...
// RegisterRoutes registers all Taxes routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/taxes", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public Taxes routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/taxes", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Taxes routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/taxes", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

...
// RegisterRoutes registers all Permissions routes
...
// RegisterPublicRoutes registers public Permissions routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/permissions", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Permissions routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/permissions", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

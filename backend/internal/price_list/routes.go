...
// RegisterRoutes registers all PriceLists routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/price_lists", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public PriceLists routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/price_lists", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only PriceLists routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/price_lists", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

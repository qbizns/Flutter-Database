...
// RegisterRoutes registers all TimeClockEntries routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/time_clock_entries", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public TimeClockEntries routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/time_clock_entries", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only TimeClockEntries routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/time_clock_entries", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

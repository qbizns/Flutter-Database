...
// RegisterRoutes registers all PosErrorLogs routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/pos_error_logs", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public PosErrorLogs routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/pos_error_logs", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only PosErrorLogs routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/pos_error_logs", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

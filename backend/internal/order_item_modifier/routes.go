...
// RegisterRoutes registers all OrderItemModifiers routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/order_item_modifiers", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public OrderItemModifiers routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/order_item_modifiers", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only OrderItemModifiers routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/order_item_modifiers", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

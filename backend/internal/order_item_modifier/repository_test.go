package order_item_modifier_test
...
func TestRepository_Create(t *testing.T) {
	repo := order_item_modifier.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestOrderItemModifiers(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestOrderItemModifiersWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestOrderItemModifiers(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestOrderItemModifiersRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestOrderItemModifiersRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/order_item_modifiers", strings.NewReader(string(body)))
...
// newTestOrderItemModifiers creates a test OrderItemModifiers entity
func newTestOrderItemModifiers(orgID uuid.UUID) *order_item_modifier.OrderItemModifiers {
	return &order_item_modifier.OrderItemModifiers{
...
// newTestOrderItemModifiersWithID creates a test OrderItemModifiers entity with specific ID
func newTestOrderItemModifiersWithID(id uuid.UUID, orgID uuid.UUID) *order_item_modifier.OrderItemModifiers {
	entity := newTestOrderItemModifiers(orgID)
	entity.ID = id
	return entity
}

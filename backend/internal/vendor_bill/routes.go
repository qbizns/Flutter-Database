...
// RegisterRoutes registers all VendorBills routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/vendor_bills", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public VendorBills routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/vendor_bills", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only VendorBills routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/vendor_bills", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

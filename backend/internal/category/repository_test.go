...
func TestRepository_Create(t *testing.T) {
	repo := category.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestCategories(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestCategoriesWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestCategories(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := category.NewRepository(testDB, testLogger)
	service := category.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateCategoriesRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestCategoriesRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestCategoriesRequest(testOrgID)
	...
	updateReq := &dto.UpdateCategoriesRequest{
		Name: &updatedValue,
	}
...
func TestHandler_Create(t *testing.T) {
	repo := category.NewRepository(testDB, testLogger)
	service := category.NewService(testDB, repo, testLogger)
	handler := category.NewHandler(service, testLogger)

	reqBody := &dto.CreateCategoriesRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/categories", strings.NewReader(string(body)))
...
// newTestCategories creates a test Categories entity
func newTestCategories(orgID uuid.UUID) *category.Categories {
...
// newTestCategoriesWithID creates a test Categories entity with specific ID
func newTestCategoriesWithID(id uuid.UUID, orgID uuid.UUID) *category.Categories {
	entity := newTestCategories(orgID)
	entity.ID = id
	return entity
}

// newTestCategoriesRequest creates a test create request
func newTestCategoriesRequest(orgID uuid.UUID) *dto.CreateCategoriesRequest {
	return &dto.CreateCategoriesRequest{

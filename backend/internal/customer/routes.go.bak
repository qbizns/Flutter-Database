...
// RegisterRoutes registers all Customers routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/customers", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public Customers routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/customers", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Customers routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/customers", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all CustomerPayments routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/customer_payments", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public CustomerPayments routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/customer_payments", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only CustomerPayments routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/customer_payments", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

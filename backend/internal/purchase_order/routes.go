...
// RegisterRoutes registers all PurchaseOrders routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/purchase_orders", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public PurchaseOrders routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/purchase_orders", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only PurchaseOrders routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/purchase_orders", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

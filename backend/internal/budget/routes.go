...
// RegisterRoutes registers all Budgets routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/budgets", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public Budgets routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/budgets", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Budgets routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/budgets", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

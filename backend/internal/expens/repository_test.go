...
func TestRepository_Create(t *testing.T) {
	repo := expens.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestExpenses(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestExpensesWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestExpenses(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := expens.NewRepository(testDB, testLogger)
	service := expens.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateExpensesRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestExpensesRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestExpensesRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := expens.NewRepository(testDB, testLogger)
	service := expens.NewService(testDB, repo, testLogger)
	handler := expens.NewHandler(service, testLogger)

	reqBody := &dto.CreateExpensesRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/expenses", strings.NewReader(string(body)))
...
// newTestExpenses creates a test Expenses entity
func newTestExpenses(orgID uuid.UUID) *expens.Expenses {
	return &expens.Expenses{
		...
		ExpenseNumber: "test_expense_number",
...
// newTestExpensesWithID creates a test Expenses entity with specific ID
func newTestExpensesWithID(id uuid.UUID, orgID uuid.UUID) *expens.Expenses {
	entity := newTestExpenses(orgID)
	entity.ID = id
	return entity
}

// newTestExpensesRequest creates a test create request
func newTestExpensesRequest(orgID uuid.UUID) *dto.CreateExpensesRequest {
	return &dto.CreateExpensesRequest{

...
// RegisterRoutes registers all Expenses routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/expenses", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public Expenses routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/expenses", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Expenses routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/expenses", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

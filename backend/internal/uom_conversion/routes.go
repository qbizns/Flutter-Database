...
// RegisterRoutes registers all UomConversions routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/uom_conversions", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public UomConversions routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/uom_conversions", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only UomConversions routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/uom_conversions", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

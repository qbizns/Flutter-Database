...
// RegisterRoutes registers all IntegrationConfigs routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/integration_configs", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public IntegrationConfigs routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/integration_configs", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only IntegrationConfigs routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/integration_configs", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

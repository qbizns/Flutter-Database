...
// RegisterRoutes registers all GeneralLedger routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/general_ledger", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public GeneralLedger routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/general_ledger", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only GeneralLedger routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/general_ledger", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

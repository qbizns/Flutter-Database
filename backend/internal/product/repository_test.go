...
func TestRepository_Create(t *testing.T) {
	repo := product.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestProducts(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestProductsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestProducts(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := product.NewRepository(testDB, testLogger)
	service := product.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateProductsRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestProductsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestProductsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := product.NewRepository(testDB, testLogger)
	service := product.NewService(testDB, repo, testLogger)
	handler := product.NewHandler(service, testLogger)

	reqBody := &dto.CreateProductsRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/products", strings.NewReader(string(body)))
...
// newTestProducts creates a test Products entity
func newTestProducts(orgID uuid.UUID) *product.Products {
...
// newTestProductsWithID creates a test Products entity with specific ID
func newTestProductsWithID(id uuid.UUID, orgID uuid.UUID) *product.Products {
	entity := newTestProducts(orgID)
	entity.ID = id
	return entity
}

// newTestProductsRequest creates a test create request
func newTestProductsRequest(orgID uuid.UUID) *dto.CreateProductsRequest {
	return &dto.CreateProductsRequest{

...
// RegisterRoutes registers all JournalEntryLines routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/journal_entry_lines", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public JournalEntryLines routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/journal_entry_lines", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only JournalEntryLines routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/journal_entry_lines", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

...
// RegisterRoutes registers all SaleReturns routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/sale_returns", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public SaleReturns routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/sale_returns", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only SaleReturns routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/sale_returns", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

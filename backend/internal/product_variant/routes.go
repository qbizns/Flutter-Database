...
// RegisterRoutes registers all ProductVariants routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/product_variants", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public ProductVariants routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/product_variants", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only ProductVariants routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/product_variants", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

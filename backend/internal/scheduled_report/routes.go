...
// RegisterRoutes registers all ScheduledReports routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/scheduled_reports", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public ScheduledReports routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/scheduled_reports", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only ScheduledReports routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/scheduled_reports", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

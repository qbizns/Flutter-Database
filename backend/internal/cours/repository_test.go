...
func TestRepository_Create(t *testing.T) {
	repo := cours.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestCourses(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestCoursesWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_Update(t *testing.T) {
	...
	entity.CourseName = "updated_value"
	...
	assert.Equal(t, "updated_value", updated.CourseName)
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestCourses(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := cours.NewRepository(testDB, testLogger)
	service := cours.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateCoursesRequest{
		
		CourseName: "test_value",
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestCoursesRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestCoursesRequest(testOrgID)
	...
	updateReq := &dto.UpdateCoursesRequest{
...
func TestHandler_Create(t *testing.T) {
	repo := cours.NewRepository(testDB, testLogger)
	service := cours.NewService(testDB, repo, testLogger)
	handler := cours.NewHandler(service, testLogger)

	reqBody := &dto.CreateCoursesRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/courses", strings.NewReader(string(body)))
...
// newTestCourses creates a test Courses entity
func newTestCourses(orgID uuid.UUID) *cours.Courses {
	return &cours.Courses{
...
// newTestCoursesWithID creates a test Courses entity with specific ID
func newTestCoursesWithID(id uuid.UUID, orgID uuid.UUID) *cours.Courses {
	entity := newTestCourses(orgID)
	entity.ID = id
	return entity
}

// newTestCoursesRequest creates a test create request
func newTestCoursesRequest(orgID uuid.UUID) *dto.CreateCoursesRequest {
	return &dto.CreateCoursesRequest{

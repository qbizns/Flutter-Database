...
// RegisterRoutes registers all DriverShifts routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/driver_shifts", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public DriverShifts routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/driver_shifts", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only DriverShifts routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/driver_shifts", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
// RegisterRoutes registers all InvoicePaymentSchedules routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/invoice_payment_schedules", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public InvoicePaymentSchedules routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/invoice_payment_schedules", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only InvoicePaymentSchedules routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/invoice_payment_schedules", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

...
// RegisterRoutes registers all LoyaltyTiers routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/loyalty_tiers", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public LoyaltyTiers routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/loyalty_tiers", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only LoyaltyTiers routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/loyalty_tiers", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

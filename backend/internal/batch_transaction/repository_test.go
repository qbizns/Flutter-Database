package batch_transaction_test
...
func TestRepository_Create(t *testing.T) {
	repo := batch_transaction.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestBatchTransactions(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestBatchTransactionsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestBatchTransactions(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestBatchTransactionsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestBatchTransactionsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/batch_transactions", strings.NewReader(string(body)))
...
// newTestBatchTransactions creates a test BatchTransactions entity
func newTestBatchTransactions(orgID uuid.UUID) *batch_transaction.BatchTransactions {
	return &batch_transaction.BatchTransactions{
...
// newTestBatchTransactionsWithID creates a test BatchTransactions entity with specific ID
func newTestBatchTransactionsWithID(id uuid.UUID, orgID uuid.UUID) *batch_transaction.BatchTransactions {
	entity := newTestBatchTransactions(orgID)
	entity.ID = id
	return entity
}

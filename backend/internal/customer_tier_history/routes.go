...
// RegisterRoutes registers all CustomerTierHistory routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/customer_tier_history", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public CustomerTierHistory routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/customer_tier_history", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only CustomerTierHistory routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/customer_tier_history", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

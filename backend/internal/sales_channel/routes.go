...
// RegisterRoutes registers all SalesChannels routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/sales_channels", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public SalesChannels routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/sales_channels", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only SalesChannels routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/sales_channels", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

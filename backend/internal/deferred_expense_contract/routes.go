...
// RegisterRoutes registers all DeferredExpenseContracts routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/deferred_expense_contracts", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public DeferredExpenseContracts routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/deferred_expense_contracts", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only DeferredExpenseContracts routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/deferred_expense_contracts", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

package product_modifier_group_test
...
func TestRepository_Create(t *testing.T) {
	repo := product_modifier_group.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestProductModifierGroups(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestProductModifierGroupsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestProductModifierGroups(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestProductModifierGroupsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestProductModifierGroupsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/product_modifier_groups", strings.NewReader(string(body)))
...
// newTestProductModifierGroups creates a test ProductModifierGroups entity
func newTestProductModifierGroups(orgID uuid.UUID) *product_modifier_group.ProductModifierGroups {
	return &product_modifier_group.ProductModifierGroups{
...
// newTestProductModifierGroupsWithID creates a test ProductModifierGroups entity with specific ID
func newTestProductModifierGroupsWithID(id uuid.UUID, orgID uuid.UUID) *product_modifier_group.ProductModifierGroups {
	entity := newTestProductModifierGroups(orgID)
	entity.ID = id
	return entity
}

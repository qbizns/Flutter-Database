...
// RegisterRoutes registers all BackgroundJobs routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/background_jobs", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public BackgroundJobs routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/background_jobs", func(r chi.Router) {
		r.Use(// middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only BackgroundJobs routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/background_jobs", func(r chi.Router) {
		// Apply middleware
		r.Use(// middlewares.AuthRequired)

...
# Add admin handler methods to all handler.go files
...
for dir in "$INTERNAL_DIR"/*/ ; do
	module=$(basename "$dir")
	handler_file="$dir/handler.go"

	# Skip if handler doesn't exist or admin methods already exist
	if [[ ! -f "$handler_file" ]] || grep -q "func (h \*Handler) AdminList" "$handler_file" 2>/dev/null; then
		continue
	fi

	# Convert module name to PascalCase for type names
	entity_name=$(echo "$module" | sed -r 's/(^|_)([a-z])/\U\2/g')

	# Generate admin methods
	cat >> "$handler_file" << 'ADMIN_METHODS_EOF'
...
ADMIN_METHODS_EOF
	...
	echo "  ✅ Added admin methods to $module"
done

echo ""
echo "✅ Added admin methods to $ADDED_COUNT modules"

...
// RegisterRoutes registers all PromotionUsage routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/promotion_usage", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public PromotionUsage routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/promotion_usage", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only PromotionUsage routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/promotion_usage", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

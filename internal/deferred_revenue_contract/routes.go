...
// RegisterRoutes registers all DeferredRevenueContracts routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/deferred_revenue_contracts", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public DeferredRevenueContracts routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/deferred_revenue_contracts", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only DeferredRevenueContracts routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/deferred_revenue_contracts", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

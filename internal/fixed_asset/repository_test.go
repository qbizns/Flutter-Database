...
func TestRepository_Create(t *testing.T) {
	repo := fixed_asset.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestFixedAssets(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestFixedAssetsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_Update(t *testing.T) {
	...
	entity.AssetNumber = "updated_value"
	...
	assert.Equal(t, "updated_value", updated.AssetNumber)
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestFixedAssets(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	...
	req := &dto.CreateFixedAssetsRequest{
		...
		AssetAccountId: uuid.New(),
		
		
		
		
		AccumulatedDepreciationAccountId: uuid.New(),
		
		
		
		
		DepreciationExpenseAccountId: uuid.New(),
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestFixedAssetsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestFixedAssetsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/fixed_assets", strings.NewReader(string(body)))
...
// newTestFixedAssets creates a test FixedAssets entity
func newTestFixedAssets(orgID uuid.UUID) *fixed_asset.FixedAssets {
	return &fixed_asset.FixedAssets{
		...
		DepreciationMethod: "test_depreciation_method",
		...
		DepreciationStartDate: time.Now(),
...
// newTestFixedAssetsWithID creates a test FixedAssets entity with specific ID
func newTestFixedAssetsWithID(id uuid.UUID, orgID uuid.UUID) *fixed_asset.FixedAssets {
	entity := newTestFixedAssets(orgID)
	entity.ID = id
	return entity
}

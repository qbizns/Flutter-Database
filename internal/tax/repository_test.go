...
func TestRepository_Create(t *testing.T) {
	repo := tax.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestTaxes(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestTaxesWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestTaxes(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := tax.NewRepository(testDB, testLogger)
	service := tax.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateTaxesRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestTaxesRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestTaxesRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := tax.NewRepository(testDB, testLogger)
	service := tax.NewService(testDB, repo, testLogger)
	handler := tax.NewHandler(service, testLogger)

	reqBody := &dto.CreateTaxesRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/taxes", strings.NewReader(string(body)))
...
// newTestTaxes creates a test Taxes entity
func newTestTaxes(orgID uuid.UUID) *tax.Taxes {
	return &tax.Taxes{
...
// newTestTaxesWithID creates a test Taxes entity with specific ID
func newTestTaxesWithID(id uuid.UUID, orgID uuid.UUID) *tax.Taxes {
	entity := newTestTaxes(orgID)
	entity.ID = id
	return entity
}

// newTestTaxesRequest creates a test create request
func newTestTaxesRequest(orgID uuid.UUID) *dto.CreateTaxesRequest {
	return &dto.CreateTaxesRequest{

...
func TestRepository_Create(t *testing.T) {
	repo := background_job.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestBackgroundJobs(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestBackgroundJobsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_Update(t *testing.T) {
	...
	assert.Equal(t, "updated_value", updated.JobType)
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestBackgroundJobs(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestBackgroundJobsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestBackgroundJobsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/background_jobs", strings.NewReader(string(body)))
...
// newTestBackgroundJobs creates a test BackgroundJobs entity
func newTestBackgroundJobs(orgID uuid.UUID) *background_job.BackgroundJobs {
	return &background_job.BackgroundJobs{
...
// newTestBackgroundJobsWithID creates a test BackgroundJobs entity with specific ID
func newTestBackgroundJobsWithID(id uuid.UUID, orgID uuid.UUID) *background_job.BackgroundJobs {
	entity := newTestBackgroundJobs(orgID)
	entity.ID = id
	return entity
}

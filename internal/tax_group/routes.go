...
// RegisterRoutes registers all TaxGroups routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/tax_groups", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public TaxGroups routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/tax_groups", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only TaxGroups routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/tax_groups", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

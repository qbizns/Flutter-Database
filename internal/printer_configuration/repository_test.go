package printer_configuration_test
...
func TestRepository_Create(t *testing.T) {
	repo := printer_configuration.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestPrinterConfigurations(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestPrinterConfigurationsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestPrinterConfigurations(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestPrinterConfigurationsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestPrinterConfigurationsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/printer_configurations", strings.NewReader(string(body)))
...
// newTestPrinterConfigurations creates a test PrinterConfigurations entity
func newTestPrinterConfigurations(orgID uuid.UUID) *printer_configuration.PrinterConfigurations {
	return &printer_configuration.PrinterConfigurations{
...
// newTestPrinterConfigurationsWithID creates a test PrinterConfigurations entity with specific ID
func newTestPrinterConfigurationsWithID(id uuid.UUID, orgID uuid.UUID) *printer_configuration.PrinterConfigurations {
	entity := newTestPrinterConfigurations(orgID)
	entity.ID = id
	return entity
}

...
func TestRepository_Create(t *testing.T) {
	repo := shift.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestShifts(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestShiftsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestShifts(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := shift.NewRepository(testDB, testLogger)
	service := shift.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateShiftsRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestShiftsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestShiftsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := shift.NewRepository(testDB, testLogger)
	service := shift.NewService(testDB, repo, testLogger)
	handler := shift.NewHandler(service, testLogger)

	reqBody := &dto.CreateShiftsRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/shifts", strings.NewReader(string(body)))
...
// newTestShifts creates a test Shifts entity
func newTestShifts(orgID uuid.UUID) *shift.Shifts {
	return &shift.Shifts{
...
// newTestShiftsWithID creates a test Shifts entity with specific ID
func newTestShiftsWithID(id uuid.UUID, orgID uuid.UUID) *shift.Shifts {
	entity := newTestShifts(orgID)
	entity.ID = id
	return entity
}

// newTestShiftsRequest creates a test create request
func newTestShiftsRequest(orgID uuid.UUID) *dto.CreateShiftsRequest {
	return &dto.CreateShiftsRequest{

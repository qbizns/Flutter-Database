...
// Validator handles TableSections validation logic
type Validator struct {
	repo *Repository
}

// NewValidator creates a new TableSections validator
...
func (v *Validator) ValidateCreate(ctx context.Context, tx pgx.Tx, req *dto.CreateTableSectionsRequest) error {
	// Basic validation
	...
	if err := v.validateSectionName(req.SectionName); err != nil {
		return err
	}
...
func (v *Validator) ValidateUpdate(ctx context.Context, tx pgx.Tx, id uuid.UUID, req *dto.UpdateTableSectionsRequest) error {
	// Basic validation
	if err := req.Validate(); err != nil {
		return fmt.Errorf("validation failed: %w", err)
	}
	...
	// Validate SectionName if provided
...
// validateSectionName validates section_name field
func (v *Validator) validateSectionName(value string) error {
	
	// Add custom validation for section_name
	if strings.TrimSpace(value) == "" {
		return fmt.Errorf("section_name cannot be empty")
	}
...
func (v *Validator) validateCrossFields(ctx context.Context, tx pgx.Tx, req *dto.CreateTableSectionsRequest) error {
...
func (v *Validator) validateBusinessRules(ctx context.Context, tx pgx.Tx, req *dto.CreateTableSectionsRequest) error {
...
func (v *Validator) validateUpdateBusinessRules(ctx context.Context, tx pgx.Tx, existing *TableSections, req *dto.UpdateTableSectionsRequest) error {
...
func (v *Validator) validateCanDelete(ctx context.Context, tx pgx.Tx, entity *TableSections) error {

...
// RegisterRoutes registers all StockAdjustmentReasons routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/stock_adjustment_reasons", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public StockAdjustmentReasons routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/stock_adjustment_reasons", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only StockAdjustmentReasons routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/stock_adjustment_reasons", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

...
func TestRepository_Create(t *testing.T) {
	repo := audit_log.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestAuditLogs(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestAuditLogsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestAuditLogs(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := audit_log.NewRepository(testDB, testLogger)
	service := audit_log.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateAuditLogsRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestAuditLogsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestAuditLogsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := audit_log.NewRepository(testDB, testLogger)
	service := audit_log.NewService(testDB, repo, testLogger)
	handler := audit_log.NewHandler(service, testLogger)

	reqBody := &dto.CreateAuditLogsRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/audit_logs", strings.NewReader(string(body)))
	...
	var resp dto.AuditLogsResponse
...
// newTestAuditLogs creates a test AuditLogs entity
func newTestAuditLogs(orgID uuid.UUID) *audit_log.AuditLogs {
	return &audit_log.AuditLogs{
...
// newTestAuditLogsWithID creates a test AuditLogs entity with specific ID
func newTestAuditLogsWithID(id uuid.UUID, orgID uuid.UUID) *audit_log.AuditLogs {
	entity := newTestAuditLogs(orgID)
	entity.ID = id
	return entity
}

// newTestAuditLogsRequest creates a test create request
func newTestAuditLogsRequest(orgID uuid.UUID) *dto.CreateAuditLogsRequest {
	return &dto.CreateAuditLogsRequest{

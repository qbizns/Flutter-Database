...
// RegisterRoutes registers all RolePermissions routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/role_permissions", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public RolePermissions routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/role_permissions", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only RolePermissions routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/role_permissions", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

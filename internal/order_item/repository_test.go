...
func TestRepository_Create(t *testing.T) {
	repo := order_item.NewRepository(testDB, testLogger)
	ctx := context.Background()

	tests := []struct {
		name    string
		entity  *order_item.OrderItems
		wantErr bool
	}{
		{
			name:    "valid entity",
			entity:  newTestOrderItems(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestOrderItemsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestOrderItems(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := order_item.NewRepository(testDB, testLogger)
	service := order_item.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateOrderItemsRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestOrderItemsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestOrderItemsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := order_item.NewRepository(testDB, testLogger)
	service := order_item.NewService(testDB, repo, testLogger)
	handler := order_item.NewHandler(service, testLogger)

	reqBody := &dto.CreateOrderItemsRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/order_items", strings.NewReader(string(body)))
	...
	var resp dto.OrderItemsResponse
...
// newTestOrderItems creates a test OrderItems entity
func newTestOrderItems(orgID uuid.UUID) *order_item.OrderItems {
	return &order_item.OrderItems{
...
// newTestOrderItemsWithID creates a test OrderItems entity with specific ID
func newTestOrderItemsWithID(id uuid.UUID, orgID uuid.UUID) *order_item.OrderItems {
	entity := newTestOrderItems(orgID)
	entity.ID = id
	return entity
}

// newTestOrderItemsRequest creates a test create request
func newTestOrderItemsRequest(orgID uuid.UUID) *dto.CreateOrderItemsRequest {
	return &dto.CreateOrderItemsRequest{

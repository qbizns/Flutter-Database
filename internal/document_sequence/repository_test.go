package document_sequence_test
...
func TestRepository_Create(t *testing.T) {
	repo := document_sequence.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestDocumentSequences(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestDocumentSequencesWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_Update(t *testing.T) {
	...
	entity.DocumentType = "updated_value"
	...
	assert.Equal(t, "updated_value", updated.DocumentType)
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestDocumentSequences(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestDocumentSequencesRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestDocumentSequencesRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/document_sequences", strings.NewReader(string(body)))
...
// newTestDocumentSequences creates a test DocumentSequences entity
func newTestDocumentSequences(orgID uuid.UUID) *document_sequence.DocumentSequences {
	return &document_sequence.DocumentSequences{
...
// newTestDocumentSequencesWithID creates a test DocumentSequences entity with specific ID
func newTestDocumentSequencesWithID(id uuid.UUID, orgID uuid.UUID) *document_sequence.DocumentSequences {
	entity := newTestDocumentSequences(orgID)
	entity.ID = id
	return entity
}

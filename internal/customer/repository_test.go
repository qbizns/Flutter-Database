...
func TestRepository_Create(t *testing.T) {
	repo := customer.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestCustomers(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestCustomersWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestCustomers(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestCustomersRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestCustomersRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/customers", strings.NewReader(string(body)))
...
// newTestCustomers creates a test Customers entity
func newTestCustomers(orgID uuid.UUID) *customer.Customers {
...
// newTestCustomersWithID creates a test Customers entity with specific ID
func newTestCustomersWithID(id uuid.UUID, orgID uuid.UUID) *customer.Customers {
	entity := newTestCustomers(orgID)
	entity.ID = id
	return entity
}

...
func TestRepository_Create(t *testing.T) {
	repo := tip_pool.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestTipPools(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestTipPoolsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestTipPools(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := tip_pool.NewRepository(testDB, testLogger)
	service := tip_pool.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateTipPoolsRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestTipPoolsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestTipPoolsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := tip_pool.NewRepository(testDB, testLogger)
	service := tip_pool.NewService(testDB, repo, testLogger)
	handler := tip_pool.NewHandler(service, testLogger)

	reqBody := &dto.CreateTipPoolsRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/tip_pools", strings.NewReader(string(body)))
...
// newTestTipPools creates a test TipPools entity
func newTestTipPools(orgID uuid.UUID) *tip_pool.TipPools {
	return &tip_pool.TipPools{
...
// newTestTipPoolsWithID creates a test TipPools entity with specific ID
func newTestTipPoolsWithID(id uuid.UUID, orgID uuid.UUID) *tip_pool.TipPools {
	entity := newTestTipPools(orgID)
	entity.ID = id
	return entity
}

// newTestTipPoolsRequest creates a test create request
func newTestTipPoolsRequest(orgID uuid.UUID) *dto.CreateTipPoolsRequest {
	return &dto.CreateTipPoolsRequest{

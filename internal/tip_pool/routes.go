...
// RegisterRoutes registers all TipPools routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/tip_pools", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public TipPools routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/tip_pools", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only TipPools routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/tip_pools", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

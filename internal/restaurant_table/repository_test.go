...
func TestRepository_Create(t *testing.T) {
	repo := restaurant_table.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestRestaurantTables(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestRestaurantTablesWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestRestaurantTables(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := restaurant_table.NewRepository(testDB, testLogger)
	service := restaurant_table.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateRestaurantTablesRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestRestaurantTablesRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestRestaurantTablesRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := restaurant_table.NewRepository(testDB, testLogger)
	service := restaurant_table.NewService(testDB, repo, testLogger)
	handler := restaurant_table.NewHandler(service, testLogger)

	reqBody := &dto.CreateRestaurantTablesRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/restaurant_tables", strings.NewReader(string(body)))
	...
	var resp dto.RestaurantTablesResponse
...
// newTestRestaurantTables creates a test RestaurantTables entity
func newTestRestaurantTables(orgID uuid.UUID) *restaurant_table.RestaurantTables {
	return &restaurant_table.RestaurantTables{
...
// newTestRestaurantTablesWithID creates a test RestaurantTables entity with specific ID
func newTestRestaurantTablesWithID(id uuid.UUID, orgID uuid.UUID) *restaurant_table.RestaurantTables {
	entity := newTestRestaurantTables(orgID)
	entity.ID = id
	return entity
}

// newTestRestaurantTablesRequest creates a test create request
func newTestRestaurantTablesRequest(orgID uuid.UUID) *dto.CreateRestaurantTablesRequest {
	return &dto.CreateRestaurantTablesRequest{

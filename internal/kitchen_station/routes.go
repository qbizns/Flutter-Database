...
// RegisterRoutes registers all KitchenStations routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/kitchen_stations", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public KitchenStations routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/kitchen_stations", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only KitchenStations routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/kitchen_stations", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

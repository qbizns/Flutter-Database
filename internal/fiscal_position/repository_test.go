...
func TestRepository_Create(t *testing.T) {
	repo := fiscal_position.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestFiscalPositions(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestFiscalPositionsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_Update(t *testing.T) {
	...
	entity.PositionCode = "updated_value"
	...
	assert.Equal(t, "updated_value", updated.PositionCode)
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestFiscalPositions(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestFiscalPositionsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestFiscalPositionsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/fiscal_positions", strings.NewReader(string(body)))
...
// newTestFiscalPositions creates a test FiscalPositions entity
func newTestFiscalPositions(orgID uuid.UUID) *fiscal_position.FiscalPositions {
	return &fiscal_position.FiscalPositions{
...
// newTestFiscalPositionsWithID creates a test FiscalPositions entity with specific ID
func newTestFiscalPositionsWithID(id uuid.UUID, orgID uuid.UUID) *fiscal_position.FiscalPositions {
	entity := newTestFiscalPositions(orgID)
	entity.ID = id
	return entity
}

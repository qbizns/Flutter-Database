...
// RegisterRoutes registers all ImmutabilityViolationsLog routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/immutability_violations_log", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public ImmutabilityViolationsLog routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/immutability_violations_log", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only ImmutabilityViolationsLog routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/immutability_violations_log", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

...
func TestRepository_Create(t *testing.T) {
	repo := currency_rate.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestCurrencyRates(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestCurrencyRatesWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestCurrencyRates(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestCurrencyRatesRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestCurrencyRatesRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/currency_rates", strings.NewReader(string(body)))
...
// newTestCurrencyRates creates a test CurrencyRates entity
func newTestCurrencyRates(orgID uuid.UUID) *currency_rate.CurrencyRates {
	return &currency_rate.CurrencyRates{
...
// newTestCurrencyRatesWithID creates a test CurrencyRates entity with specific ID
func newTestCurrencyRatesWithID(id uuid.UUID, orgID uuid.UUID) *currency_rate.CurrencyRates {
	entity := newTestCurrencyRates(orgID)
	entity.ID = id
	return entity
}

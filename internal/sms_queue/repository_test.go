...
func TestRepository_Create(t *testing.T) {
	repo := sms_queue.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestSmsQueue(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestSmsQueueWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_Update(t *testing.T) {
	...
	assert.Equal(t, "updated_value", updated.ToPhone)
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestSmsQueue(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := sms_queue.NewRepository(testDB, testLogger)
	service := sms_queue.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateSmsQueueRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestSmsQueueRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestSmsQueueRequest(testOrgID)
	...
	updateReq := &dto.UpdateSmsQueueRequest{
		ToPhone: &updatedValue,
	}
...
func TestHandler_Create(t *testing.T) {
	repo := sms_queue.NewRepository(testDB, testLogger)
	service := sms_queue.NewService(testDB, repo, testLogger)
	handler := sms_queue.NewHandler(service, testLogger)

	reqBody := &dto.CreateSmsQueueRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/sms_queue", strings.NewReader(string(body)))
...
// newTestSmsQueue creates a test SmsQueue entity
func newTestSmsQueue(orgID uuid.UUID) *sms_queue.SmsQueue {
	return &sms_queue.SmsQueue{
...
// newTestSmsQueueWithID creates a test SmsQueue entity with specific ID
func newTestSmsQueueWithID(id uuid.UUID, orgID uuid.UUID) *sms_queue.SmsQueue {
	entity := newTestSmsQueue(orgID)
	entity.ID = id
	return entity
}

// newTestSmsQueueRequest creates a test create request
func newTestSmsQueueRequest(orgID uuid.UUID) *dto.CreateSmsQueueRequest {
	return &dto.CreateSmsQueueRequest{

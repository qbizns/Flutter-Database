...
func RegisterRoutes(r chi.Router, handler *Handler) {
	...
	r.Route("/api/v1/organizations/{orgID}/promotions", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public Promotions routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/promotions", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only Promotions routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/promotions", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

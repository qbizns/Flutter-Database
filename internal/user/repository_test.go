...
func TestRepository_Create(t *testing.T) {
	repo := user.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestUsers(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestUsersWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_Update(t *testing.T) {
	...
	assert.Equal(t, "updated_value", updated.Email)
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestUsers(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := user.NewRepository(testDB, testLogger)
	service := user.NewService(testDB, repo, testLogger)
	ctx := context.Background()
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestUsersRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestUsersRequest(testOrgID)
	...
	updateReq := &dto.UpdateUsersRequest{
		Email: &updatedValue,
	}
...
func TestHandler_Create(t *testing.T) {
	repo := user.NewRepository(testDB, testLogger)
	service := user.NewService(testDB, repo, testLogger)
	handler := user.NewHandler(service, testLogger)

	reqBody := &dto.CreateUsersRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/users", strings.NewReader(string(body)))
...
// newTestUsers creates a test Users entity
func newTestUsers(orgID uuid.UUID) *user.Users {
	return &user.Users{
...
// newTestUsersWithID creates a test Users entity with specific ID
func newTestUsersWithID(id uuid.UUID, orgID uuid.UUID) *user.Users {
	entity := newTestUsers(orgID)
	entity.ID = id
	return entity
}

// newTestUsersRequest creates a test create request
func newTestUsersRequest(orgID uuid.UUID) *dto.CreateUsersRequest {
	return &dto.CreateUsersRequest{

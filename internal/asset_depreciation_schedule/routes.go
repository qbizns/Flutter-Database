...
// RegisterRoutes registers all AssetDepreciationSchedule routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/asset_depreciation_schedule", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public AssetDepreciationSchedule routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/asset_depreciation_schedule", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only AssetDepreciationSchedule routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/asset_depreciation_schedule", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

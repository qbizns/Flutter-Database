...
// Validator handles BatchTransactions validation logic
type Validator struct {
	repo *Repository
}

// NewValidator creates a new BatchTransactions validator
...
func (v *Validator) ValidateCreate(ctx context.Context, tx pgx.Tx, req *dto.CreateBatchTransactionsRequest) error {
	// Basic validation
	...
	if err := v.validateBatchIdExists(ctx, tx, req.BatchId); err != nil {
		return err
	}
...
func (v *Validator) ValidateUpdate(ctx context.Context, tx pgx.Tx, id uuid.UUID, req *dto.UpdateBatchTransactionsRequest) error {
	// Basic validation
	if err := req.Validate(); err != nil {
		return fmt.Errorf("validation failed: %w", err)
	}
...
// validateBatchIdExists validates that batch_id exists
func (v *Validator) validateBatchIdExists(ctx context.Context, tx pgx.Tx, id uuid.UUID) error {
	// TODO: Implement existence check for batch
	// This should query the referenced table to ensure the ID exists
	var exists bool
	err := tx.QueryRow(ctx, "SELECT EXISTS(SELECT 1 FROM batch WHERE id = $1)", id).Scan(&exists)
	if err != nil {
		return fmt.Errorf("failed to check batch existence: %w", err)
	}
	if !exists {
		return fmt.Errorf("batch with id %s does not exist", id)
	}
	return nil
}
...
func (v *Validator) validateCrossFields(ctx context.Context, tx pgx.Tx, req *dto.CreateBatchTransactionsRequest) error {
...
func (v *Validator) validateBusinessRules(ctx context.Context, tx pgx.Tx, req *dto.CreateBatchTransactionsRequest) error {
...
func (v *Validator) validateUpdateBusinessRules(ctx context.Context, tx pgx.Tx, existing *BatchTransactions, req *dto.UpdateBatchTransactionsRequest) error {
...
func (v *Validator) validateCanDelete(ctx context.Context, tx pgx.Tx, entity *BatchTransactions) error {

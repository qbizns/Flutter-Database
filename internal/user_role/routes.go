...
// RegisterRoutes registers all UserRoles routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Global routes (no organization scope)
	r.Route("/api/v1/user_roles", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public UserRoles routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/user_roles", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only UserRoles routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/user_roles", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

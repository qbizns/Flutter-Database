...
// RegisterRoutes registers all OrderTrackingEvents routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/order_tracking_events", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public OrderTrackingEvents routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/order_tracking_events", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only OrderTrackingEvents routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/order_tracking_events", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

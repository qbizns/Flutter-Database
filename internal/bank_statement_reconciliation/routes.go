...
// RegisterRoutes registers all BankStatementReconciliations routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/bank_statement_reconciliations", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public BankStatementReconciliations routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/bank_statement_reconciliations", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only BankStatementReconciliations routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/bank_statement_reconciliations", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

...
func TestRepository_Create(t *testing.T) {
	repo := modifier.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestModifiers(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestModifiersWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestModifiers(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := modifier.NewRepository(testDB, testLogger)
	service := modifier.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateModifiersRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestModifiersRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestModifiersRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := modifier.NewRepository(testDB, testLogger)
	service := modifier.NewService(testDB, repo, testLogger)
	handler := modifier.NewHandler(service, testLogger)

	reqBody := &dto.CreateModifiersRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/modifiers", strings.NewReader(string(body)))
	...
	var resp dto.ModifiersResponse
...
// newTestModifiers creates a test Modifiers entity
func newTestModifiers(orgID uuid.UUID) *modifier.Modifiers {
	return &modifier.Modifiers{
		...
		ModifierName: "test_modifier_name",
...
// newTestModifiersWithID creates a test Modifiers entity with specific ID
func newTestModifiersWithID(id uuid.UUID, orgID uuid.UUID) *modifier.Modifiers {
	entity := newTestModifiers(orgID)
	entity.ID = id
	return entity
}

// newTestModifiersRequest creates a test create request
func newTestModifiersRequest(orgID uuid.UUID) *dto.CreateModifiersRequest {
	return &dto.CreateModifiersRequest{

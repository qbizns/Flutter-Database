...
func TestRepository_Create(t *testing.T) {
	repo := order.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestOrders(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestOrdersWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestOrders(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := order.NewRepository(testDB, testLogger)
	service := order.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateOrdersRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestOrdersRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestOrdersRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := order.NewRepository(testDB, testLogger)
	service := order.NewService(testDB, repo, testLogger)
	handler := order.NewHandler(service, testLogger)

	reqBody := &dto.CreateOrdersRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/orders", strings.NewReader(string(body)))
...
// newTestOrders creates a test Orders entity
func newTestOrders(orgID uuid.UUID) *order.Orders {
	return &order.Orders{
...
// newTestOrdersWithID creates a test Orders entity with specific ID
func newTestOrdersWithID(id uuid.UUID, orgID uuid.UUID) *order.Orders {
	entity := newTestOrders(orgID)
	entity.ID = id
	return entity
}

// newTestOrdersRequest creates a test create request
func newTestOrdersRequest(orgID uuid.UUID) *dto.CreateOrdersRequest {
	return &dto.CreateOrdersRequest{

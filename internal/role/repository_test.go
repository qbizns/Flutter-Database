...
func TestRepository_Create(t *testing.T) {
	repo := role.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestRoles(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestRolesWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestRoles(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestRolesRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestRolesRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/roles", strings.NewReader(string(body)))
...
// newTestRoles creates a test Roles entity
func newTestRoles(orgID uuid.UUID) *role.Roles {
	return &role.Roles{
...
// newTestRolesWithID creates a test Roles entity with specific ID
func newTestRolesWithID(id uuid.UUID, orgID uuid.UUID) *role.Roles {
	entity := newTestRoles(orgID)
	entity.ID = id
	return entity
}

...
// RegisterRoutes registers all FileAttachments routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/file_attachments", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public FileAttachments routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/file_attachments", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only FileAttachments routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/file_attachments", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)

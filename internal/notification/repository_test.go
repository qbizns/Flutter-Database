...
func TestRepository_Create(t *testing.T) {
	repo := notification.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestNotifications(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestNotificationsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestNotifications(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := notification.NewRepository(testDB, testLogger)
	service := notification.NewService(testDB, repo, testLogger)
	ctx := context.Background()
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestNotificationsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestNotificationsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := notification.NewRepository(testDB, testLogger)
	service := notification.NewService(testDB, repo, testLogger)
	handler := notification.NewHandler(service, testLogger)

	reqBody := &dto.CreateNotificationsRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/notifications", strings.NewReader(string(body)))
	...
	var resp dto.NotificationsResponse
...
// newTestNotifications creates a test Notifications entity
func newTestNotifications(orgID uuid.UUID) *notification.Notifications {
...
// newTestNotificationsWithID creates a test Notifications entity with specific ID
func newTestNotificationsWithID(id uuid.UUID, orgID uuid.UUID) *notification.Notifications {
	entity := newTestNotifications(orgID)
	entity.ID = id
	return entity
}

// newTestNotificationsRequest creates a test create request
func newTestNotificationsRequest(orgID uuid.UUID) *dto.CreateNotificationsRequest {
	return &dto.CreateNotificationsRequest{

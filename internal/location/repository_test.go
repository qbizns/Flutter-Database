...
func TestRepository_Create(t *testing.T) {
	repo := location.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestLocations(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestLocationsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_Update(t *testing.T) {
	...
	entity.LocationCode = "updated_value"
	...
	assert.Equal(t, "updated_value", updated.LocationCode)
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestLocations(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := location.NewRepository(testDB, testLogger)
	service := location.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateLocationsRequest{
		
		LocationCode: "test_value",
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestLocationsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestLocationsRequest(testOrgID)
	...
	updateReq := &dto.UpdateLocationsRequest{
		LocationCode: &updatedValue,
	}
...
func TestHandler_Create(t *testing.T) {
	repo := location.NewRepository(testDB, testLogger)
	service := location.NewService(testDB, repo, testLogger)
	handler := location.NewHandler(service, testLogger)

	reqBody := &dto.CreateLocationsRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/locations", strings.NewReader(string(body)))
	...
	var resp dto.LocationsResponse
...
// newTestLocations creates a test Locations entity
func newTestLocations(orgID uuid.UUID) *location.Locations {
...
// newTestLocationsWithID creates a test Locations entity with specific ID
func newTestLocationsWithID(id uuid.UUID, orgID uuid.UUID) *location.Locations {
	entity := newTestLocations(orgID)
	entity.ID = id
	return entity
}

// newTestLocationsRequest creates a test create request
func newTestLocationsRequest(orgID uuid.UUID) *dto.CreateLocationsRequest {
	return &dto.CreateLocationsRequest{

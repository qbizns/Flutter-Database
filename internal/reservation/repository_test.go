...
func TestRepository_Create(t *testing.T) {
	repo := reservation.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestReservations(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestReservationsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestReservations(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := reservation.NewRepository(testDB, testLogger)
	service := reservation.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateReservationsRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestReservationsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestReservationsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := reservation.NewRepository(testDB, testLogger)
	service := reservation.NewService(testDB, repo, testLogger)
	handler := reservation.NewHandler(service, testLogger)

	reqBody := &dto.CreateReservationsRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/reservations", strings.NewReader(string(body)))
	...
	var resp dto.ReservationsResponse
...
// newTestReservations creates a test Reservations entity
func newTestReservations(orgID uuid.UUID) *reservation.Reservations {
...
// newTestReservationsWithID creates a test Reservations entity with specific ID
func newTestReservationsWithID(id uuid.UUID, orgID uuid.UUID) *reservation.Reservations {
	entity := newTestReservations(orgID)
	entity.ID = id
	return entity
}

// newTestReservationsRequest creates a test create request
func newTestReservationsRequest(orgID uuid.UUID) *dto.CreateReservationsRequest {
	return &dto.CreateReservationsRequest{

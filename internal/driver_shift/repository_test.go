...
func TestRepository_Create(t *testing.T) {
	repo := driver_shift.NewRepository(testDB, testLogger)
	...
	tests := []struct {
		...
		{
			name:    "valid entity",
			entity:  newTestDriverShifts(testOrgID),
			wantErr: false,
		},
		{
			name:    "duplicate primary key",
			entity:  newTestDriverShiftsWithID(uuid.New(), testOrgID),
			wantErr: true,
		},
	}
...
func TestRepository_ListByOrganization(t *testing.T) {
	...
	otherOrgID := uuid.New()
	for i := 0; i < 2; i++ {
		entity := newTestDriverShifts(otherOrgID)
		err := repo.Create(ctx, tx, entity)
		require.NoError(t, err)
	}
...
func TestService_Create(t *testing.T) {
	repo := driver_shift.NewRepository(testDB, testLogger)
	service := driver_shift.NewService(testDB, repo, testLogger)
	ctx := context.Background()

	req := &dto.CreateDriverShiftsRequest{
...
func TestService_GetByID(t *testing.T) {
	...
	// Create test entity
	req := newTestDriverShiftsRequest(testOrgID)
...
func TestService_Update(t *testing.T) {
	...
	// Create test entity
	createReq := newTestDriverShiftsRequest(testOrgID)
...
func TestHandler_Create(t *testing.T) {
	repo := driver_shift.NewRepository(testDB, testLogger)
	service := driver_shift.NewService(testDB, repo, testLogger)
	handler := driver_shift.NewHandler(service, testLogger)

	reqBody := &dto.CreateDriverShiftsRequest{
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/organizations/"+testOrgID.String()+"/driver_shifts", strings.NewReader(string(body)))
	...
	var resp dto.DriverShiftsResponse
...
// newTestDriverShifts creates a test DriverShifts entity
func newTestDriverShifts(orgID uuid.UUID) *driver_shift.DriverShifts {
	return &driver_shift.DriverShifts{
...
// newTestDriverShiftsWithID creates a test DriverShifts entity with specific ID
func newTestDriverShiftsWithID(id uuid.UUID, orgID uuid.UUID) *driver_shift.DriverShifts {
	entity := newTestDriverShifts(orgID)
	entity.ID = id
	return entity
}

// newTestDriverShiftsRequest creates a test create request
func newTestDriverShiftsRequest(orgID uuid.UUID) *dto.CreateDriverShiftsRequest {
	return &dto.CreateDriverShiftsRequest{

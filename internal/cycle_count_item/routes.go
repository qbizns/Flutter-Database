...
// RegisterRoutes registers all CycleCountItems routes
func RegisterRoutes(r chi.Router, handler *Handler) {
	
	// Organization-scoped routes
	r.Route("/api/v1/organizations/{orgID}/cycle_count_items", func(r chi.Router) {
		// Apply middleware
...
// RegisterPublicRoutes registers public CycleCountItems routes (if any)
func RegisterPublicRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/public/cycle_count_items", func(r chi.Router) {
		r.Use(middlewares.RateLimiter)
		r.Use(middleware.Compress(5))
...
// RegisterAdminRoutes registers admin-only CycleCountItems routes
func RegisterAdminRoutes(r chi.Router, handler *Handler) {
	r.Route("/api/v1/admin/cycle_count_items", func(r chi.Router) {
		// Apply middleware
		r.Use(middlewares.AuthRequired)
